...
import { GitMerge, Play, Save, Plus, X, FlaskConical } from 'lucide-react'
...
import { hasAbEpdAdjustments } from '../engine/abEpd'
...
export default function PredictionsPage() {
  ...
  function runPrediction(selectedSire: Animal | null, selectedDam: Animal | null) {
    ...
    try {
      const weights = Object.keys(customWeights).length > 0 ? customWeights : undefined
      let result
      if (
        sBreed.id !== dBreed.id &&
        hasAbEpdAdjustments(sBreed.id) &&
        hasAbEpdAdjustments(dBreed.id)
      ) {
        const angus = getBreedConfig('angus')
        if (!angus) throw new Error('Angus reference config not found')
        result = predictProgenyAbEpd(selectedSire, selectedDam, sBreed, dBreed, angus, { customWeights: weights })
      } else {
        result = predictProgeny(selectedSire, selectedDam, sBreed, { customWeights: weights })
      }
  ...
  return (
    <section className="card">
      ...
      <div className="card-body space-y-6">
        {prediction.isAbEpdAdjusted && (
          <div className="rounded-md border border-blue-200 bg-blue-50 px-4 py-3 flex items-start gap-2 text-sm text-blue-900">
            <FlaskConical size={16} className="shrink-0 mt-0.5 text-blue-600" />
            <span>
              <strong>Cross-Breed Estimate — USMARC 2024 AB-EPD Adjusted.</strong>{' '}
              Values for BW, WW, YW, Milk, Marb, RE, and Fat have been converted
              to a common Angus-base scale using USDA-ARS USMARC 2024 Across-Breed
              EPD adjustment factors. Percentile ranks are relative to the Angus
              population. Results are approximate and best used for ranking, not exact comparison.
            </span>
          </div>
        )}
