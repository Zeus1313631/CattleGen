 ...
 *   with other breeds.
 *
 * CROSS-BREED EPD NOTE: predictProgenyAbEpd() applies USMARC 2024 AB-EPD
 * adjustment factors (see abEpd.ts) to convert both parents' EPDs to the
 * common Angus-base scale before parent averaging. Supported breeds: Angus,
 * Red Angus, Hereford, Simmental, Limousin, Brahman, Beefmaster.
 */
...
import { AB_EPD_TRAIT_KEYS, applyAbEpdAdjustments, hasAbEpdAdjustments } from './abEpd'
...
// ----------------------------------------------------------------------------
// 1b. predictProgenyAbEpd — cross-breed prediction via USMARC 2024 AB-EPDs
// ----------------------------------------------------------------------------

/**
 * Parent-average prediction for sire and dam from different EPD breeds.
 * Both breeds must have USMARC 2024 AB-EPD adjustment factors (see abEpd.ts).
 *
 * Steps:
 *  1. Normalize each parent's EPDs to the standard USDA scale.
 *  2. Apply breed-specific AB-EPD adjustment factors (→ Angus-base scale).
 *  3. Parent-average the 7 common AB-EPD traits.
 *  4. Rank results against the referenceBreed (Angus) percentile ranges.
 *
 * @param referenceBreed - Angus BreedConfig; provides percentile ranges and averages.
 */
export function predictProgenyAbEpd(
  sire: Animal,
  dam: Animal,
  sireBreed: BreedConfig,
  damBreed: BreedConfig,
  referenceBreed: BreedConfig,
  options: { customWeights?: Record<string, number> } = {}
): MatingPrediction {
  if (sireBreed.evaluationSystem !== 'EPD' || damBreed.evaluationSystem !== 'EPD') {
    throw new CrossEvaluationSystemError(sireBreed.evaluationSystem, damBreed.evaluationSystem)
  }
  if (!hasAbEpdAdjustments(sireBreed.id) || !hasAbEpdAdjustments(damBreed.id)) {
    throw new Error(
      `AB-EPD adjustments not available for "${sireBreed.id}" or "${damBreed.id}".`
    )
  }

  // Normalize each parent's raw EPDs to the standard USDA scale
  const sireNorm: Record<string, number | null> = {}
  const damNorm: Record<string, number | null> = {}
  for (const key of AB_EPD_TRAIT_KEYS) {
    sireNorm[key] = normalizeForEngine(sire.epds[key], key, sireBreed.marblingScaleMultiplier)
    damNorm[key] = normalizeForEngine(dam.epds[key], key, damBreed.marblingScaleMultiplier)
  }

  // Shift both to the common Angus-base AB-EPD scale
  const sireAdj = applyAbEpdAdjustments(sireNorm, sireBreed.id)
  const damAdj = applyAbEpdAdjustments(damNorm, damBreed.id)

  const predictedProgeny: Record<string, number | null> = {}
  const percentileRankings: Record<string, number> = {}

  for (const key of AB_EPD_TRAIT_KEYS) {
    const sv = sireAdj[key]
    const dv = damAdj[key]
    // Fallback: Angus breed average (already on the reference scale, no multiplier needed)
    const avg = normalizeForEngine(referenceBreed.breedAverages[key], key, undefined)

    let progeny: number | null
    if (sv != null && dv != null) {
      progeny = (sv + dv) / 2
    } else if (sv != null && avg != null) {
      progeny = (sv + avg) / 2
    } else if (dv != null && avg != null) {
      progeny = (dv + avg) / 2
    } else {
      progeny = null
    }
    predictedProgeny[key] = progeny

    if (progeny != null) {
      const refTrait = referenceBreed.traits.find((t) => t.key === key)
      const range = referenceBreed.percentileRanges[key]
      if (range && refTrait) {
        percentileRankings[key] = estimatePercentile(progeny, range, refTrait.higherIsBetter)
      }
    }
  }
